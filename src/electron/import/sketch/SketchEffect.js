...
export default {
  ...
  addBitmapEffects (filterEffects, element) {
    ...
    this.addFilterValue('brightness', controls.brightness + 1, 1, filterEffects) // -1, 0, 1 <--> 0, 1, 2
    this.addFilterValue('saturate', controls.saturation, 1, filterEffects) // 0, 1, 2 <--> 0, 1, 2
    this.addFilterValue('contrast', controls.contrast, 1, filterEffects) // 0, 1, 4 <--> 0, 1, 4
    this.addFilterValue('hue-rotate', controls.hue + 'rad', '0rad', filterEffects) // -3.14, 0, 3.14 <--> -3.14rad, 0rad, 3.14rad
  },
  ...
  getFilter (effect, data) {
    if (data.method === 'blur') {
      if (effect.type !== 0) return null // 0 = gaussian blur, 3 = background blur
  ...
  getBoxShadow (effect) {
    ...
    return {
      'box-shadow': `${data.color} ${data.x}px ${data.y}px ${data.radius}px ${data.spread}px${inner}`
    }
  },
